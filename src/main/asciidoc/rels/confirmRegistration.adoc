...
This link is used to confirm user registration.

Confirmation process is done via email message. Recently registered user to confirm that given while registration e-mail belongs to him, needs to check his inbox and find a message sent from forum, where he can confirm registration which is equivalent to activating the account. Default, user has 7 days to confirm the registration.
